...
const bookList = [
    ...
    {'primary' : [
        ...
        {id : 45, name : "VERBAL REASONING FOR PRIMARY BOOK 4", info :"<b>ISBN</b>: 978-978-50937-8-0" , image :"vrp4.png"},
...
const requestList = async () => {
  ...
  try {
    ...
    if (myRequests.length !== 0) {
      ...
      let verfyRequiTab = document.createElement('table');
      verfyRequiTab.style.backgroundColor = 'white';
      let VtBody = document.createElement('tbody');
      let VtHead = document.createElement('tr');
      VtHead.innerHTML = `
        <th>Date</th>
        <th>cost </th>
      `;
      VtBody.appendChild(VtHead);

      let hasDraft = false;
      let isPending = false;
      let selDate;

      for (const key of myRequests) {
        if (typeof key === 'object' && key !== null) {
          const invoice_date = key.date;
          const invoice_status = key.status;
          selDate = invoice_date;
          const reqitem = key.ref;
          ...
          switch (invoice_status) {
            case 'verified':
              ...
              row.innerHTML = `
                ...
                <td>₦${addCommasToNumber(await valueRequisition(reqitem))}</td>
              `;
              sumdCost += Number(await valueRequisition(key.ref));
      ...
      verfyRequiTab.appendChild(VtBody);
      ...
      appBody.innerHTML = `
        ...
        <div style="text-transform: none; background: #f7f7f7; box-shadow: 0px 0px 3px #c5c5c5; margin: 2px auto; padding-bottom: 0.5rem;">
          <div class="list">
            verified requisition
            <span> ₦${addCommasToNumber(alex)} <i class="bi bi-arrow-down-short"></i></span>
          </div>
          <div id="veriTab" style="padding: 1rem" class="dropdown-content myDropdown"></div>
        </div>
      ...
      document.querySelector('#veriTab').appendChild(verfyRequiTab);
...
const valueRequisition = async(ref) =>{
       // Retrieve the array from storage 
       const storedOrder = await window.get1Request(ref);
       let myOrder = [];
       if (storedOrder) {
    ...
    if (typeof myOrder === 'object' && myOrder !== null){
        ...
        if (Array.isArray(cartItemsAry) && cartItemsAry.length > 0){
            ...
            //console.log(indexToRemove);
                ...
                let row = document.createElement('tr');
                row.innerHTML = `<td id="${book_id}|${book_class} | ${indexToRemove}" style="background: white; text-align: left; box-shadow: 0px 0px 3px red inset;" name="rmRq">${item.book}</td> 
                                <td class="qnt_text" contentEditable="true" style="box-shadow: 0px 0px 3px rgb(0, 140, 255) inset;">${item.qnt}</td> 
                               `;
                tBody.appendChild(row);
